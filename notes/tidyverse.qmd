...
```{r}
#| eval: false
species_group <- starwars %>%
  group_by(species) %>%
  arrange(desc(height))

slice(species_group, 1:2)
slice_head(species_group, n = 2)
slice_tail(species_group, n = 2)
slice_max(species_group, height)
slice_min(species_group, height)
slice_sample(species_group, n = 2)
```
