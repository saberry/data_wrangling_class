...
nba_data[, fg_vars] <- nba_data[,  lapply(.SD, function(x) x * 100), .SDcols = fg_vars]
...
```{r}
sum(top_pt_performance$PLAYER_NAME %in% all_players$full_name)

top_performance <- all_players[top_pt_performance, on = "full_name==PLAYER_NAME"]
```
...
top_performance$plot_name <- gsub("\\s", "\n", top_performance$full_name)

ggplot(top_performance, aes(weight_pounds, PTS, color = player_game_number, 
                        label = plot_name)) +
  geom_text() +
  theme_minimal()
```
